...
// Fetch pending booking requests for the logged-in artist
$requests = [];
if (isset($artist['artist_id'])) {
    $stmt = $link->prepare("
        ...
        WHERE b.artist_id = ? AND b.status = 'pending'
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['booking_id'])) {
    ...
    $stmt = $link->prepare("UPDATE booking SET status = ? WHERE booking_id = ? AND artist_id = ?");
    $stmt->bind_param("sii", $new_status, $booking_id, $artist['artist_id']);
    $stmt->execute();
    $stmt->close();
    
    // Redirect to refresh the page
    header("Location: artistrequests.php");
    exit();
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <link rel="stylesheet" href="artistrequests.css">
            ...
            <div class="requests-container">
                <table class="requests-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="10" class="no-requests">No pending requests found</td>
                        </tr>
                        <?php else: ?>
                            <?php foreach ($requests as $request): ?>
                            <tr>
                                <td>#BK<?php echo str_pad($request['booking_id'], 3, '0', STR_PAD_LEFT); ?></td>
                                <td><?php echo htmlspecialchars($request['organizer_name']); ?></td>
                                <td><?php echo htmlspecialchars($request['event_location']); ?></td>
                                <td><?php echo htmlspecialchars($request['occasion']); ?></td>
                                <td><?php echo htmlspecialchars($request['event_time']); ?></td>
                                <td><?php echo htmlspecialchars($request['event_date']); ?></td>
                                <td><?php echo htmlspecialchars($request['participant_count']); ?></td>
                                <td><?php echo htmlspecialchars($request['package_type']); ?></td>
                                <td>Rs. <?php echo number_format($request['total_amount'], 2); ?></td>
                                <td class="action-buttons">
                                    <form method="POST" style="display: inline;">
                                        ...
                                        <button type="submit" name="action" value="reject" class="cancel-btn">
