...
// Fetch pending booking requests for the logged-in organizer
$requests = [];
if (isset($_SESSION['organizer_id'])) {
    $stmt = $link->prepare("
        ...
        WHERE b.organizer_id = ? AND b.status = 'pending'
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cancel_booking'])) {
    ...
    $stmt = $link->prepare("UPDATE booking SET status = 'cancelled' WHERE booking_id = ? AND organizer_id = ?");
    $stmt->bind_param("ii", $booking_id, $_SESSION['organizer_id']);
    
    if ($stmt->execute()) {
        header("Location: organizerrequests.php");
        exit();
    }
    $stmt->close();
}
...
<head>
    ...
    <link rel="stylesheet" href="organizerrequests.css">
                        ...
                        <tbody>
                            <?php foreach ($requests as $request): ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($request['artist_name']); ?></td>
                                    <td><?php echo htmlspecialchars($request['occasion']); ?></td>
                                    <td><?php echo date('Y-m-d', strtotime($request['event_date'])); ?></td>
